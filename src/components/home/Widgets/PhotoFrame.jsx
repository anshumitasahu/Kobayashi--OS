...
export default function PhotoFrame() {
    ...
    if (photos.length === 0) {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center gap-1.5 rounded-xl bg-neutral-900 p-3 text-center">
                <ImageIcon size={22} className="text-neutral-500" />
                <p className="text-xs font-semibold text-neutral-300">No photos yet</p>
                <p className="text-[11px] text-neutral-500">Take one with the Camera app</p>
            </div>
        );
    }

    return (
        <div className="relative w-full h-full rounded-xl overflow-hidden bg-neutral-900 group/frame">
            {current ? (
                ...
                <button
                    ...
                    className="w-full h-full flex flex-col items-center justify-center gap-1.5 text-neutral-300 hover:text-white"
                >
                    <ImageIcon size={22} />
            ...
            {current && !picking && (
                <>
                    ...
                    <button
                        onMouseDown={(e) => e.stopPropagation()}
                        onClick={() => setPicking(true)}
                        className="absolute bottom-1.5 left-1/2 -translate-x-1/2 rounded-full bg-black/40 px-2.5 py-1 text-[11px] font-medium text-white opacity-0 group-hover/frame:opacity-100 transition-opacity"
                    >
                        Change
                    </button>
                </>
            ...
            {picking && (
                <div className="absolute inset-0 bg-black/80 backdrop-blur-sm p-2 overflow-y-auto">
                    <p className="text-[11px] font-semibold text-white mb-1.5 px-0.5">Choose a photo</p>
                    ...
                    {current && (
                        <button
                            ...
                            className="mt-1.5 w-full rounded-md bg-white/15 py-1 text-[11px] text-white"
                        >
                            Cancel
                        </button>
                    )}
